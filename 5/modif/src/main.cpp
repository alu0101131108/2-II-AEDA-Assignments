...
typedef std::vector<std::vector<SSN>> SSN_testbench;
...
void runVisualMode()
{
  ...
  std::cin >> algorithm;
  std::cout << "Comparisons follow the next notation: ¿{{SSN}} < ((SSN))? \nEnter any char to continue with iterations.\n";

  std::vector<SSN> ssn_test;
  for (int i = 0; i < size; i++)
    ssn_test.push_back(SSN());

  switch (algorithm)
  {
    case INSERTIONSORT:
      demInsertionSort(ssn_test, size);
      break;

    case BUBBLESORT:
      demBubbleSort(ssn_test, size);
      break;

    case HEAPSORT:
      demHeapSort(ssn_test, size);
      break;

    case QUICKSORT:
      demFullQuickSort(ssn_test, size);
      break;
    ...
    case SHELLSORT:
      ...
      demShellSort(ssn_test, size, shellSort_alpha);
      break;
...
void runStatisticMode()
{
  ...
  {
    model.push_back(std::vector<SSN>());
    for (int j = 0; j < size; j++)
      model[i].push_back(SSN());
  }

  SSN_testbench insertSort_tb = model;
  SSN_testbench bubbleSort_tb = model;
  SSN_testbench heapSort_tb = model;
  SSN_testbench quickSort_tb = model;
  SSN_testbench shellSort_tb = model;
